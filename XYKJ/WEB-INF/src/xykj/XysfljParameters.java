 ...
 *
 *  修正履歴
 *  L001  2006/05/11 K.Sasaki   ログ出力対応　ログ出力メソッドのコメントアウトと新規追加
 *  L002  2006/06/09 O.Ogawara  サービス時間外の場合、コネクションクローズ処理追加
 *
 */
package xykj;
			...
			if( COMMAND_DOWNLOAD != commandTypeStatus ){
				...
				if( null != request.getParameter("screenNm") ){
					switch( new Integer( request.getParameter("screenNm") ).intValue() ){
						case 010: ScreenNm = XysfljGenericRules.SCREEN_NM_010; break;
						case 110: ScreenNm = XysfljGenericRules.SCREEN_NM_110; break;
						case 120: ScreenNm = XysfljGenericRules.SCREEN_NM_120; break;
						case 130: ScreenNm = XysfljGenericRules.SCREEN_NM_130; break;
						case 210: ScreenNm = XysfljGenericRules.SCREEN_NM_210; break;
...
//							(String)request.getParameter("objectNm"));						// 画面PGM名称
				}
			}
		}catch(Exception e){
			exit(); // クローズ漏れ
...
// ******** No.L001-2 ****************************  END  ***********************************************
	...
	public String createHiddenTag(String n,String v){
		...
		tag.append(n);
		tag.append("' value='");
		tag.append(Xysflj_Common.htmlEnc(v));
		tag.append("'>\r\n");
		return tag.toString();
	}
...
//		}
		...
		Enumeration obj = msg_box.elements();
		while(obj.hasMoreElements()){
			String m = (String)obj.nextElement();
			sb.append("msg_print(\"");
			sb.append("\",\"");
			sb.append(m);
			sb.append("\")\r\n");
...
//		e.printStackTrace();
	...
	public void exit(){
		...
		if(null != masterDbConnection){
			try{
				System.out.println(this.getClass().getName() + ".exit:" + masterDbConnection.getConnection().hashCode());
				Logger logger = Logger.getLogger(this.getClass().getName());
				logger.info(this.getClass().getName() + ".exit:"+ masterDbConnection.getConnection().hashCode());
		...
		if(null != slaveDbConnection){
			try{
				System.out.println(this.getClass().getName() + ".exit:" + slaveDbConnection.getConnection().hashCode());
				Logger logger = Logger.getLogger(this.getClass().getName());
				logger.info(this.getClass().getName() + ".exit:"+ slaveDbConnection.getConnection().hashCode());
