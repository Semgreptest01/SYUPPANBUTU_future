 ...
 *  L002  2006/07/05 K.Sasaki   SQLのチューニング
 *  L003  2006/11/08 M.Saitou   発売日２重表示対応
 *
 */
package xykj;
...
public class Xykj0130DetailTable extends XysfljTableTag {
	...
	private Xykjlj_01301_Params param;
...
//	private int cursor = 0;
	...
	public Xykj0130DetailTable(Xykjlj_01301_Params p) {
		super("Xykj0130DetailTable", p);
		param = p;
		...
		titleCol = new InputDataCol("", 85, "D"); //発売日２重表示対応
		sumCol = new InputDataCol("合計", 76, "E"); //発売日２重表示対応
		...
		//検索以外の場合は処理を行わない
		if ((Xykjlj_01301_Params.COMMAND_REFERENCE == ct)
			|| (Xykjlj_01301_Params.COMMAND_DOWNLOAD == ct)) {
...
//			ResultSet rsMax = tdb.selectTable(sqlMax);
			...
			ResultSet rsMax = tdb.selectTable(sqlMax, params);
			//MAX（最大）経過日数の取得
			while ( rsMax.next()) {
...
//			ResultSet rs = tdb.selectTable(sql);
			String[][] params2 = new String[2][2];
			params2[0][0] = Xysflj_Dbcom.DB_STR;
			params2[0][1] = select_publish;
			params2[1][0] = Xysflj_Dbcom.DB_STR;
			params2[1][1] = select_publish;

			ResultSet rs = tdb.selectTable(sql, params2);
...
//				if (!(brand.equals(rs.getString("銘柄名")))
	...
	public void getNewTotalDataFromDB() throws XysfljDB.DBException {
		try {
				...
				StringBuffer add_tori =new StringBuffer();
				StringBuffer add_mei =new StringBuffer();
				mk_list.append("\'");
...
//				sql2 +=		"AND B.取引先コード  in ("+( torihiki_list )+") ";
				sql2 += "WHERE A.出版社コード = ? ";
				sql2 +=		"AND B.取引先コード  in ("+ add_tori.toString() +") ";
				sql2 += 	"AND A.銘柄コード = B.商品コード ";											
							
//				ResultSet rs2 = tdb.selectTable(sql2); // SQL実行
				int size = TorihikiCD.size() / 2;
				size++;
				...
				params[0][1] = select_publish;
				int idx = 1;
				for(int cnt2 = 0; cnt2<TorihikiCD.size() / 2; cnt2++) {
					int idx2 = cnt2 * 2;
					params[idx][0] = Xysflj_Dbcom.DB_STR;
					params[idx][1] = TorihikiCD.elementAt(idx2).toString();
					idx++;
				}
...
//			sqlMax +=   "AND A.銘柄コード in ("+( meigara_list)+") ";
			sqlMax += "WHERE A.出版社コード = ? ";
			sqlMax += 	"AND B.取引先コード in ("+ add_tori.toString() +") ";
			sqlMax +=   "AND A.銘柄コード in ("+ add_mei.toString() +") ";
			sqlMax += 	"AND A.銘柄コード = B.商品コード ";

//			ResultSet rsMax = tdb.selectTable(sqlMax);
			int size2 = size + br.size();
			String[][] params2 = new String[size2][2];
			params2[0][0] = Xysflj_Dbcom.DB_STR;
			params2[0][1] = select_publish;
			int idx2 = 1;
			for(int cnt2 = 0; cnt2<TorihikiCD.size() / 2; cnt2++) {
				int idx3 = cnt2 * 2;
				params2[idx2][0] = Xysflj_Dbcom.DB_STR;
				params2[idx2][1] = TorihikiCD.elementAt(idx3).toString();
				idx2++;
			}
			for(int cnt2 = 0; cnt2<br.size(); cnt2++) {
				params2[idx2][0] = Xysflj_Dbcom.DB_STR;
				params2[idx2][1] = br.elementAt(cnt2).toString();
				idx2++;
			}
			
			ResultSet rsMax = tdb.selectTable(sqlMax, params2);
			//MAX（最大）経過日数の取得
...
//			sql	+=	" AND A.銘柄コード in ("+( meigara_list )+") ";
			sql	+=	" AND A.銘柄コード in ("+ add_mei.toString() +") ";
			sql +=  " AND A.銘柄コード = B.銘柄コード ";
			sql +=	" AND A.ＪＡＮコード = B.ＪＡＮコード "; //発売日２重表示対応
			sql	+=   " AND A.県コード = '00'";
			sql	+=   "  AND A.発売日 = B.発売日 ";
...
//			ResultSet rs = tdb.selectTable(sql);
			...
			String[][] params3 = new String[size3][2];
			params3[0][0] = Xysflj_Dbcom.DB_STR;
			params3[0][1] = select_publish;
			int idx3 = 1;
			for(int cnt2 = 0; cnt2<br.size(); cnt2++) {
				params3[idx3][0] = Xysflj_Dbcom.DB_STR;
				params3[idx3][1] = br.elementAt(cnt2).toString();
				idx3++;
			}
			
			ResultSet rs = tdb.selectTable(sql, params3);
...
//				if (!(brand.equals(rs.getString("銘柄名")))
	...
	public String createStartTagWithFrame(String f) {
		...
		*/
		tag.append("<TABLE");
		int ds = 120 +70 + 90 + 85 + 70 + 70*maxday;
		setStyle( 
		...
		tag.append("\t<COL style=\"width:120px\">\r\n");
		tag.append("\t<COL style=\"width:70px\">\r\n");
		tag.append("\t<COL style=\"width:90px\">\r\n"); //発売日２重表示対応
		...
		while (records.hasMoreElements()) {
			...
			while (cnt < maxday + 5) { //発売日２重表示対応
				...
				if (i == 1) { // 行数が1行目の場合のみrowspanを設定
					//2列目まで縦方向にセルを連結
					if (cnt < 3) { //発売日２重表示対応
//					if ((cnt < 3) && (cnt!=2)){ //発売日２重表示対応
						String value = data.get(l.getName());
						tag.append(
							"\t\t\t<TD rowspan=" + rowspan);
						tag.append(" " + l.getOptionTag(value,cursor,data));
						if (!cls2.isEmpty())tag.append(getCls2());
						tag.append(" style=\"padding:" + cellpadding + ";text-align:center\">\r\n");
		...
		while (cursor < minLines) {
			...
			while (column.hasMoreElements()) {
				...
				if (false == l.rowspanCheck()) {
					//tag.append("\t\t\t<TD style=\"padding:" + cellpadding + ";\"><BR></TD>\r\n");
					tag.append("\t\t\t<TD style=\"padding:" + cellpadding + ";\"");
					if (!cls2.isEmpty())tag.append(getCls2());
					tag.append("><BR></TD>\r\n");
				}
