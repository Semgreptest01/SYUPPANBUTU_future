...
public class Xykj0210CheckTable extends XysfljTableTag {
	...
	public Xykjlj_02101_Params param = null;
	...
	public Xykj0210CheckTable(Xykjlj_02101_Params p) {
		super("Xykj0210CheckTable", p);
		param = p;
...
//		fromCol = new InputDataCol2("From", 135, "B");
		labelCol = new InputDataCol("Label", 190, "A");
	...
	public void getTotalDataFromDB() throws XysfljDB.DBException {
		try {
			...
			sql += " SELECT  to_char(A.発売日,'YYYY/MM/DD') saledate   "
				...
				+	"FROM UPM_新聞社別商品 A "
				+		",UPM_新聞社 B "
				+	"WHERE A.新聞社コード = B.新聞社コード "
//				+		" AND A.商品コード =" + select_Productform  ;
			+		" AND A.商品コード = ? "  ;
...
//		tag.append("\t<COL style=\"width:200px\">\r\n");
		tag.append("\t<COL style=\"width:190px\">\r\n");
...
//	            	csv.add("新聞社：" + strfrom.substring(0,6) + " " + strfrom.substring(7));
...
//				chk2_loss_type = rs.getString("商品名");
				...
				// **** E10K移行時変更対応(2006/06/08) ***
				setFromCol( Xysflj_Common.getStrCvt( rs.getString("商品名") ));
//				setFromCol(rs.getString("商品名"));
				// **** END ******************************
