...
public class Xykj0210DetailTable extends XysfljTableTag {
	...
	private Xykjlj_02101_Params param;
...
//	private int cursor = 0;
	...
	public Xykj0210DetailTable(Xykjlj_02101_Params p) {
		super("Xykj0210DetailTable", p);
		param = p;
		...
		notitleCol = new InputDataCol("", 76, "C");
		...
		//コンボBOXで選択された値をgetする
		select_DEPT = param.getValue(XykjljNewsDeptClassCombo.NAME);
		select_SUB = param.getValue(XykjljNewsSUBCLASSCombo.NAME);
		select_Issueform = param.getValue(XykjljNewsISSUEFORMCombo.NAME);
		select_Productform = param.getValue(XykjljNewsProductCombo.NAME);
		
		//検索以外の場合は処理を行わない
		if ((Xykjlj_02101_Params.COMMAND_REFERENCE == ct)
			|| (Xykjlj_02101_Params.COMMAND_DOWNLOAD == ct)) {
...
//			rs = tdb.selectTable(sql);
	...
	public String createStartTagWithFrame(String f) {
		...
		StringBuffer tag = new StringBuffer();
		tag.append("<TABLE");
		int ds = 60 +60  + 85 + 70 + 50*param.printDays;
...
// 無駄コード（ページ制御）
//		int maxPage = tableElements.size() / 9;
//		if ( tableElements.size() % 9 != 0 ) {
//			maxPage++;
//		}
//		int curPage = 0;
...
// 無駄コード（ページ制御）
			tag.append("<DIV id=\"BODY1\" style=\"height: 410px; width:995px;  overflow:auto;\">");
			tag.append(createStartTag());
			TableData data = null;
			int pageLineCnt = 0;
			while (records.hasMoreElements()) {
