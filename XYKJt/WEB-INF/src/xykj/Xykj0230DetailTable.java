 ...
 *
 *  修正履歴
 *  L001  2006/07/05 K.Sasaki   SQLのチューニング
 */
 package xykj;
...
public class Xykj0230DetailTable extends XysfljTableTag {
	...
	private Xykjlj_02301_Params param;
...
//	private int cursor = 0;
	...
	public Xykj0230DetailTable(Xykjlj_02301_Params p) {
		super("Xykj0230DetailTable", p);
		param = p;
		...
		//コンボBOXで選択された値をgetする
		select_publish = param.getValue(XykjljNewsPUBLISHCOMCombo.NAME);
...
//			System.out.println("取引先コードは" + TorihikiCD);
		...
		//検索以外の場合は処理を行わない
		if ((Xykjlj_02301_Params.COMMAND_REFERENCE == ct)
			|| (Xykjlj_02301_Params.COMMAND_DOWNLOAD == ct)) {
...
//				System.out.println("pro=" + pro);
				...
				StringBuffer add_pro =new StringBuffer();
...
//				+   " AND C.取引先コード in ("+( torihiki_list )+") "
				+   " AND C.取引先コード in ("+ add_tori.toString() +") "
				+	" AND A.商品コード = B.商品コード "
				+	" AND A.商品コード = C.商品コード "
				+	" AND A.県コード = 00 "
				+	" AND to_char(B.発売日,'YYYYMMDD') > '" + day + "'";
...
//			ResultSet rs3 = tdb.selectTable(sql3); // SQL実行
			...
			ResultSet rs3 = tdb.selectTable(sql3, params2); // SQL実行
...
//			sql	+=	" AND A.商品コード in ("+( syouhin_list )+") ";
			sql	+=	" AND A.商品コード in ("+ add_pro.toString() +") ";
...
//			rs = tdb.selectTable(sql);
			...
			int idx3 = 1;
			for(int cnt2 = 0; cnt2<pro.size(); cnt2++) {
				params3[idx3][0] = Xysflj_Dbcom.DB_STR;
				params3[idx3][1] = pro.elementAt(cnt2).toString();
				idx3++;
			}

			rs = tdb.selectTable(sql, params3);
	...
	public String createTag() throws NoDataException, SpanException {
		...
		if (((Xykjlj_02301_Params.COMMAND_REFERENCE == ct)
			...
			if ( Xykjlj_02301_Params.COMMAND_REFERENCE == ct ) {
				...
				tag.append("<DIV id=\"BODY1\" style=\"height: 410px; width:990px; overflow:auto;\">");
				tag.append(createStartTag());
	...
	public String createStartTagWithFrame(String f) {
		...
		StringBuffer tag = new StringBuffer();
		tag.append("<TABLE");
		int ds = 115 + 85 + 70 + 50*param.printDays;
		setStyle( 
