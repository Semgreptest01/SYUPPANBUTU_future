...
//				ResultSet rs2 = tdb.selectTable(sql2); // SQL実行
				...
				size2++;
				String[][] params = new String[size2][2];
				params[0][0] = Xysflj_Dbcom.DB_STR;
				params[0][1] = select_publish;
				int idx2 = 1;
				for(int cnt2 = 0; cnt2<TorihikiCD.size() / 2; cnt2++) {
					...
					params[idx2][0] = Xysflj_Dbcom.DB_STR;
					params[idx2][1] = TorihikiCD.elementAt(idx3).toString();
					idx2++;
...
//			ResultSet rs3 = tdb.selectTable(sql3); // SQL実行
			int size3 = TorihikiCD.size() / 2;
			size3+=2;
			
			String[][] params2 = new String[size3][2];
			params2[0][0] = Xysflj_Dbcom.DB_STR;
			params2[0][1] = select_publish;
			params2[1][0] = Xysflj_Dbcom.DB_STR;
			params2[1][1] = select_publish;
			...
			for(int cnt2 = 0; cnt2<TorihikiCD.size() / 2; cnt2++) {
			int idx4 = cnt2 * 2;
				params2[idx3][0] = Xysflj_Dbcom.DB_STR;
				params2[idx3][1] = TorihikiCD.elementAt(idx4).toString();
				idx3++;
			}
...
//			rs = tdb.selectTable(sql);
			...
			String[][] params3 = new String[size4][2];
			params3[0][0] = Xysflj_Dbcom.DB_STR;
			params3[0][1] = select_publish;
			int idx4 = 1;
			for (int cnt2 = 0; cnt2< pro.size(); cnt2++) {
				params3[idx4][0] = Xysflj_Dbcom.DB_STR;
				params3[idx4][1] = pro.elementAt(cnt2).toString();
				idx4++;
			}
	...
	public String createTag() throws NoDataException, SpanException {
		...
		if (((Xysklj_02301_Params.COMMAND_REFERENCE == ct)
			...
			if ( Xysklj_02301_Params.COMMAND_REFERENCE == ct ) {
				...
				tag.append("<DIV id=\"BODY1\" style=\"height: 410px; width:983px; overflow:auto;\">");
				tag.append(createStartTag());
	...
	public String createStartTagWithFrame(String f) {
		...
		StringBuffer tag = new StringBuffer();
		tag.append("<TABLE");
		int ds = 110 + 85 + 70 + 50*param.printDays;
		setStyle(
		...
		tag.append("\t<COL style=\"width:110px\">\r\n");
...
//XysfljGenericRules.outputMessage("vYoubi.size()="+ vYoubi.size());
	...
	public String createDataTag() throws NoDataException, SpanException {
		...
		while (records.hasMoreElements()) {
			...
			if (!cls3.isEmpty())tag.append(getCls3());
